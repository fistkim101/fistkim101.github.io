---
layout: article
title: NEXTSTEP 인프라공방 - (미션 6-1) 조회 성능 개선하기
tags: Infra DB MySQL
---
...
group By 에 사용되는 컬럼에 인덱스를 걸어두면 결과적으로 정렬되어 있는 컬럼을 그룹별 탐색 하는 것이 되므로 속도가 빨라진다.
**(INDEX는 항상 키 컬럼 순으로 정렬된 상태를 유지한다)**
...
{% include image.html src="/assets/images/infra/sql_2.png" %}
...
hospital 테이블에 code, name 순으로 인덱스를 걸어주면 해당 쿼리에 대해서는 인덱스가 모든 정보를 이미 갖고 있으므로
테이블 엑세스 없이 데이터 출력이 가능하다 **(coverd index)**
...
{% include image.html src="/assets/images/infra/sql_3.png" %}
...
{% include image.html src="/assets/images/infra/sql_4.png" %}
...
{% include image.html src="/assets/images/infra/sql_5.png" %}
