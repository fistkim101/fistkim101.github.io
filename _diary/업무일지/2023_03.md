...
- [ ] emetal 연동 작업 완료 및 서버 재배포 후 테스트
...
- [x] emetal 스마트 컨트랙트 연동
  ...
  - [ ] 화면에 경매 응답 모델이 transactionHash, contractAddress 추가해서 화면에서 아래 기능 작업하기.
    - [ ] 어떤 경우든 transactionHash 는 존재하므로 이더 스캔 링크를 경매 카드에 걸어주어서 사용자가 직접 이더스캔 상에서 확인할 수 있도록 한다. 
    - [ ] transactionHash 만 있고 contractAddress 가 없는 경우, pending 이라는 것을 인지시키도록 dialog 작업을 한다.
    - [ ] transactionHash 와 contractAddress 모두 있는 경우 백엔드 api 호출하여 contractAddress 를 이용해서 auctionData 를 블록체인 네트워크에서 받아와서 프론트에서 dialog 로 보여주되, 해당 데이터가 원천이 블록체인 네트워크임을 명시한다.
