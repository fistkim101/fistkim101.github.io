...
### 2023. 01. 16
- [ ] 경매 종료시 version 차이에 따른 에러 캐칭하여 정상케이스로 넘기고 알람등 나가지 않도록 바로 return 처리 로직 추가 (0.5h)
- [ ] 전체 문서화 및 리팩토링 완료되면 테스트코드 전체 다시 싱크 확인 (0.5h)
- [ ] 시세 UX 관련 리팩토링하기(현재 UX가 너무나 완성도를 떨어트린다) (1h)
- [ ] admin-app 모듈 security 설정 및 기본 configuration 설정 (0.5h)
